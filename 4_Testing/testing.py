...
politics_DF = spark.read.text('/home/hadoop/spark/Testing/Politics/*')
politics_DF = politics_DF.withColumn("category",lit("politics"))
business_DF = spark.read.text('/home/hadoop/spark/Testing/Business/*')
business_DF = business_DF.withColumn("category",lit("business"))
sports_DF = spark.read.text('/home/hadoop/spark/Testing/Sports/*')
sports_DF = sports_DF.withColumn("category",lit("sports"))
technology_DF = spark.read.text('/home/hadoop/spark/Testing/Technology/*')
technology_DF = technology_DF.withColumn("category",lit("technology"))
...
testdata = final_DF.select([column for column in final_DF.columns])
data.show(10)
...
trainingData = pipelineFit.transform(data)

pipelineFitTest = pipeline.fit(testdata)
testData = pipelineFitTest.transform(testdata)
